		...
		public void HighlightHCore(List<GraphNode> hCoreNodes, string clickedNodeId)
		{
			...
			Application.Current.Dispatcher.Invoke(() =>
			{
				foreach (Node node in viewer.Graph.Nodes)
				{
					...
					{
						NodeStyleService.ApplyNodeStyle(node, GraphColorPalette.ClickedNodeColor, citationCount); // KIRMIZI
					}
					else if (currentHCoreNodes.Contains(node.Id))
					{
						NodeStyleService.ApplyNodeStyle(node, GraphColorPalette.HCoreNodeColor, citationCount); // MAVİ
					}
					else
					{
						NodeStyleService.ApplyNodeStyle(node, GraphColorPalette.DefaultNodeColor, citationCount); // GRİ
					}
				}
